...
export function rentaAFormulario(renta: Renta, esVestidos = false): RentaFormValues {
  ...
  return {
    ...
    detallesSaco: renta.detallesSaco ?? '',
    detallesChaleco: renta.detallesChaleco ?? '',
    detallesPantalon: renta.detallesPantalon ?? '',
...
export function formularioAPayload(
  ...
  } else if (anticipo <= 0 && precio > 0 && !esPagoEnUsd(metodoPago)) {
    anticipo = precio
  }
